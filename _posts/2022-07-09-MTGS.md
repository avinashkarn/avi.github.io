...
Based on my preliminary analysis, MT-GS models gave same prediction accuracies as UT-GS models for most the evaluated triats, however, prediction accuracy of the grain yield increased to ~2% with MT-GS models, which was interesting, since it is known that yield has low heritability and is controlled by a large number of small-effect quantitative trait loci.
